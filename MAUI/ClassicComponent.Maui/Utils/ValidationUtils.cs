...
public class Validation
{
	public static async Task<PermissionStatus> CheckAndRequestCameraPermission()
	{
		...
		if (status == PermissionStatus.Denied && DeviceInfo.Platform == DevicePlatform.iOS)
		{
			...
			await ShowMessage("Validation Issue: Camera permission",
								"Permission Denied. Please change permission in the application settings.");
		...
		if (Permissions.ShouldShowRationale<Permissions.Camera>())
		{
			...
			await ShowMessage("Validation Issue: Camera permission",
								"Please grant permission to access the camera in order to proceed.");
		}

    ...
    public class ImageListController : UIViewController
    {
        ...
        private async Task PerformOCRAndCreatePDFAsync(NSUrl[] inputUrls, NSUrl outputUrl)
        {
            ...
            try
            {
                ...
                var (ocrResult, outputPdfUrl) = await DocumentUtilities.PerformOCRAsync(ocrRecognizer: recognizer, inputUrls: inputUrls, outputUrl: outputUrl, shouldGeneratePdf: true, encrypter: ScanbotUI.DefaultImageStoreEncrypter);
                if (ocrResult != null)
                {
